...
class Vista:
    def __init__(self, root):
        ...
        # Frame para la URL y los botones Abrir Carpeta y Eliminar Texto
        self.url_btns_frame = tk.Frame(self.frame)
        self.url_btns_frame.pack()

        # Cargar y redimensionar la imagen del icono - Boton Pegar
        self.icon_pegar = Image.open("/media/libardo/567E18C87E18A333/IDEAFIX/DescargasYoutube/Repositorio/youtubedownload/descargas_youtube/image/pegar.png")  # Reemplaza "ruta/a/tu/imagen/icono.png" con la ruta de tu propia imagen
        ...
        self.icon_borrar = Image.open("/media/libardo/567E18C87E18A333/IDEAFIX/DescargasYoutube/Repositorio/youtubedownload/descargas_youtube/image/eliminar.png")  # Reemplaza "ruta/a/tu/imagen/icono.png" con la ruta de tu propia imagen
        ...
        # Frame para los Frame: Inicial - Final
        ...
        self.time_label_inicial = tk.Label(self.lbl_time_inicial_frame, text="Tiempo Inicial", font=("Arial", 12, 'bold underline'))
        ...
        self.time_label_final = tk.Label(self.lbl_time_final_frame, text="Tiempo Final", font=("Arial", 12, 'bold underline'))
        ...
        # Frame para los botones: PEGAR - DESCARGAR - BORRAR
        self.btn_frame = tk.Frame(self.frame)
        self.btn_frame.pack(pady=30)

        # Cargar y redimensionar la imagen del icono - Boton Descargar
        self.icon_descargar = Image.open("/media/libardo/567E18C87E18A333/IDEAFIX/DescargasYoutube/Repositorio/youtubedownload/descargas_youtube/image/descargar.png")  # Reemplaza "ruta/a/tu/imagen/icono.png" con la ruta de tu propia imagen
            ...
            try:
                ...
                if audio:
                    ...
                    if self.controls_tiempo_inicial.get() and self.controls_tiempo_final.get():
                        ...
                        ffmpeg.input(archivo_mp4_ruta).output(archivo_mp3_ruta, audio_bitrate="256k", ss=start_time, to=end_time).run()
                    elif self.controls_tiempo_inicial.get():
                        min_inicio = self.min_slider_inicio.get()
                        sec_inicio = self.sec_slider_inicio.get()
                        start_time = self.tiempo_inicial_func(min_inicio, sec_inicio)
                        ffmpeg.input(archivo_mp4_ruta).output(archivo_mp3_ruta, audio_bitrate="256k", ss=start_time).run()
                    elif self.controls_tiempo_final.get():
                        min_final = self.min_slider_final.get()
                        sec_final = self.sec_slider_final.get()
                        end_time = self.tiempo_final_func(min_final, sec_final)
                        ffmpeg.input(archivo_mp4_ruta).output(archivo_mp3_ruta, audio_bitrate="256k", to=end_time).run()
                    else:
                        ffmpeg.input(archivo_mp4_ruta).output(archivo_mp3_ruta, audio_bitrate="256k").run()
